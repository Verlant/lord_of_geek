...
class C_Client
{
    public function inscription(String $mail, String $pseudo, String $mdp, String $nomPrenom): void
    {
    ...
    public function creerAdresseLivraison(String $adresse, String $nom, String $ville, String $cp, C_Session $session): void
    {
        ...
        if (M_CodePostal::trouveLeCodePostal($cp) == false) {
            M_CodePostal::creerCodePostal($cp, $ville_id);
        } else {
            $cp = M_CodePostal::trouveLeCodePostal($cp, $ville_id)['id'];
        }

        $client_id = $session::getIdClient();
        M_AdresseLivraison::creerAdresseLivraison($adresse,  $nom,  $cp, $client_id);
